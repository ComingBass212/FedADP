...
if __name__ == '__main__':
    ...
    if args.withDP:
        try:
            inspector = ModuleValidator()
            inspector.validate(global_model)
            print("Model's already Valid!\n")
        except:
            global_model = inspector.fix(global_model)
            inspector = ModuleValidator()
   ...
   # for u in range(args.num_users):
    ...
    if args.withDP:
        ...
        data_loader = DataLoader(train_dataset, batch_size=int(actual_train_ds_size * args.sampling_prob))
        global_privacy_engine = PrivacyEngine()
        model, optimizer, data_loader = global_privacy_engine.make_private(
            module = global_model,
        ...
        for u in idxs_users:
            ...
            if args.withDP:
                ...
                model, optimizer, data_loader = privacy_engine.make_private(
                    module=local_models[u],
                    optimizer=args.optimizer,
                    data_loader=local_model.trainloader,
                    max_grad_norm=args.max_grad_norm,
        ...
        if epoch != 0 and epoch % 30 == 0:
            ...
            if args.withDP:
             ...
             #   epsilons, _ = global_privacy_engine.get_privacy_spent(args.delta)
                epsilons, _ = global_privacy_engine.get_epsilon(args.delta)
